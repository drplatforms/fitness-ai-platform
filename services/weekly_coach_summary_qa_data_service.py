...
def qa_date_range_cache_key(
    user_id: int, start_date: date | str, end_date: date | str
) -> str:
...
def _completed_workouts_from_inventory(inventory: WeeklyCoachSummaryQAInventory) -> int:
    return max(
        inventory.fact_counts.get("workout_sessions", 0),
        inventory.fact_counts.get("workout_execution_sessions", 0),
        int(inventory.completed_counts.get("workout_execution_sessions") or 0),
...
def build_weekly_summary_context_from_qa_range(
    *,
    ...
    limitations.append(
        f"QA Date Range Debug inventory source: {inventory.source}; data quality: {inventory.data_quality_label}."
    )
    if (
