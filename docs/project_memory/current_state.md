...
The Developer Mode Weekly Coach Summary QA Date Range Debug path now builds/uses a backend-owned selected-range `WeeklyCoachSummaryContext` with safe aggregate facts, selected user/date provenance, source metadata, data quality, limitations, and reason codes. User 102 for `2026-06-08` through `2026-06-14` remains the happy path, while user 105 for the same range remains the low-data/fallback path.
