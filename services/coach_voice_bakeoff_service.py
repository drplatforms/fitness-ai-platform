...
def build_coach_voice_prompt(context: CoachVoiceContext) -> str:
    ...
    payload = {
        "task": "Write one bounded coach note for AI Health Coach.",
        "rules": [
            "Return strict JSON only.",
            "Use only approved_facts.",
            "Do not invent foods, exercises, serving sizes, calorie targets, macro targets, recovery claims, progression claims, medical claims, or new actions.",
            "recommended_focus must exactly match one approved_focus_options item.",
            "used_approved_facts must contain exact strings copied from approved_facts.",
            "Keep coach_note concise enough for a UI card.",
        ],
        "schema": COACH_VOICE_JSON_SCHEMA,
