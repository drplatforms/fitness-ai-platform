...
def build_direct_ollama_nutrition_report_section_prompt(
    ...
    return (
        ...
        "practical_food_focus rules: if approved_food_suggestions contains items, "
        "use only those approved suggestions. You may mention exact display_name "
        "values and exact suggested_grams only when they appear in the context, or "
        "you may say to choose from the approved food suggestions without naming "
        "a food. Do not add other foods, servings, substitutions, supplements, "
        "or meal plans. If approved_food_suggestions is empty, say no approved "
        "food suggestion is available from the current evidence, or say logging "
        "completeness must improve before a specific food suggestion is safe.\n"
