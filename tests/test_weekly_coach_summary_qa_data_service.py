...
def create_qa_range_fixture_db(path: Path) -> None:
    ...
    connection.executescript("""
        ...
        CREATE TABLE daily_checkins (id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, checkin_date TEXT NOT NULL, notes TEXT);
        CREATE TABLE food_entries (id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, food_id INTEGER NOT NULL, grams REAL NOT NULL, entry_date TEXT NOT NULL);
        CREATE TABLE workout_sessions (id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, workout_date TEXT NOT NULL);
        CREATE TABLE workout_plan_instances (
            id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, status TEXT NOT NULL,
            scenario TEXT NOT NULL, confidence TEXT NOT NULL, title TEXT NOT NULL,
            approved_workout_plan_json TEXT NOT NULL, selected_at TEXT, created_at TEXT
        );
        CREATE TABLE planned_workout_exercises (
            id INTEGER PRIMARY KEY, workout_plan_instance_id INTEGER NOT NULL,
            exercise_order INTEGER NOT NULL, name TEXT NOT NULL, sets INTEGER NOT NULL,
            reps_min INTEGER NOT NULL, reps_max INTEGER NOT NULL, rir_min INTEGER NOT NULL,
            rir_max INTEGER NOT NULL, notes TEXT NOT NULL, equipment_required_json TEXT NOT NULL
        );
        CREATE TABLE workout_execution_sessions (
            id INTEGER PRIMARY KEY, workout_plan_instance_id INTEGER NOT NULL,
            user_id INTEGER NOT NULL, status TEXT NOT NULL, workout_session_id INTEGER,
            started_at TEXT, completed_at TEXT, created_at TEXT
        );
        CREATE TABLE workout_execution_set_actuals (
            id INTEGER PRIMARY KEY, workout_execution_session_id INTEGER NOT NULL,
            workout_session_id INTEGER, exercise_name TEXT NOT NULL, set_number INTEGER NOT NULL,
    ...
    for day in range(8, 15):
        ...
        connection.execute(
            "INSERT INTO food_entries (user_id, food_id, grams, entry_date) VALUES (?, ?, ?, ?)",
...
def test_qa_range_defaults_are_stable() -> None:
    ...
    assert QA_DATE_RANGE_PRESETS["latest_seeded_week"] == ("2026-06-08", "2026-06-14")
...
def test_qa_range_preset_and_cache_key_are_typed() -> None:
    ...
    assert (
        qa_date_range_cache_key(102, start, end)
        == "user:102|start:2026-06-08|end:2026-06-14"
    )


def test_inspect_weekly_summary_qa_range_returns_safe_inventory(tmp_path: Path) -> None:
    db_path = tmp_path / "qa.db"
    create_qa_range_fixture_db(db_path)
    inventory = inspect_weekly_summary_qa_range(
        user_id=102, start_date="2026-06-08", end_date="2026-06-14", db_path=db_path
    )
...
def test_low_data_user_inventory_is_limited_but_safe(tmp_path: Path) -> None:
    ...
    create_qa_range_fixture_db(db_path)
    inventory = inspect_weekly_summary_qa_range(
        user_id=105, start_date="2026-06-08", end_date="2026-06-14", db_path=db_path
    )
...
def test_out_of_range_inventory_warns_with_available_bounds(tmp_path: Path) -> None:
    ...
    create_qa_range_fixture_db(db_path)
    inventory = inspect_weekly_summary_qa_range(
        user_id=102, start_date="2030-01-01", end_date="2030-01-07", db_path=db_path
    )
